...
For a longer write-up of the navigation stack and EC2 helpers, see the [repository root README](../README.md).
...
| **Python env** | **Conda**; the provided installer creates env `behavior` with **Python 3.10** (see [setup.sh](setup.sh)). |
...
If the simulator loads stale USD or behaves oddly after asset edits, clear Kit caches as described in the [root README](../README.md) (global cache and scene JSON USDC artifacts).
...
- **Stale or corrupted sim state** — Remove Kit / appdata caches and scene USD caches (see [root README](../README.md)).
- **DCV “localhost:8443” unreachable** — Ensure [dcv_tunnel.sh](../dcv_tunnel.sh) (or an equivalent `ssh -L`) is running.
- **Killing the wrong process over SSH** — Avoid `pkill -f` patterns that match your shell; prefer a process-specific pattern (see [root README](../README.md)).

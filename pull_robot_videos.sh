#!/usr/bin/env bash
# Pull output2/robot_videos/ from EC2 into this repo (reverse rsync for MP4/AVI outputs).
#
# From repo root on your Mac:
#   chmod +x pull_robot_videos.sh
#   ./pull_robot_videos.sh -i ~/.ssh/key.pem ubuntu@EC2_HOST
#
...
#
# Optional: REMOTE_DIR=MyTakeHome ./pull_robot_videos.sh ...
#
set -euo pipefail
...
REMOTE_PATH="${REMOTE}:~/${REMOTE_NAME}/output2/robot_videos/"
LOCAL_PATH="$HERE/output2/robot_videos/"

mkdir -p "$LOCAL_PATH"
"${SSH_BASE[@]}" -o BatchMode=yes "$REMOTE" "mkdir -p \"\$HOME/${REMOTE_NAME}/output2/robot_videos\""

#!/usr/bin/env python3
"""
Autonomous navigation for 180 continuous simulation seconds in OmniGibson — Benevolence_1_int.
...
# Extra trav-map blocker rects in world meters (x0, y0, x1, y1), painted blocked
# after paint_object_obstacles. Use for regions where an object's reported AABB
# under-represents its collision footprint.
EXTRA_TRAV_BLOCKERS_M: list[tuple[float, float, float, float]] = [
    # sofa_uixwiv_0 NE corner: robot clips here on X1 -> X2 (contact ~(1.47, 2.80))
    (0.50, 2.30, 1.70, 2.95),
]
...
NAV_CSV_NAME = "autonomous_nav_benevolence1_nav.csv"
EVENTS_LOG_NAME = "autonomous_nav_benevolence1_events.log"
SUMMARY_JSON_NAME = "autonomous_nav_benevolence1_summary.json"
CSV_FLUSH_EVERY = 30
...
# Preset waypoints (world x, y, label) for Benevolence_1_int — red X centroids from
# benevolence1_marked.pdf via plot_marked_waypoints_path.py (see output/benevolence1_marked_path.png).
# Visit left->right on the marked map. Runtime snap_to_reachable() may adjust slightly per chain step.
...
# 14 was too wide (carved furniture); 6 was too tight on Benevolence_1_int (disconnected rooms on EC2).
DOORWAY_RADIUS_CELLS = 12

# Open archways — trav-map paint at corridor bends + arch openings (no door objects in scene).
...
# Spawn candidates: prefer marked tour start (X1) so snap chain can include X1→X2→X3; include offsets
# ~0.5–1.0 m from X1 so spawn_near_goals can satisfy min distance to PRESET_GOALS[0] (not inside goal disk).
...
def spawn_near_goals(env, robot, floor: int, floor_z: float):
    ...
    Prefer a spawn that reaches PRESET_GOALS[0] first so the snapped tour can chain X1→X2→X3.
    """
...
def paint_doorways_from_doors(env, floor: int = 0, free_mask: np.ndarray | None = None) -> int:
    """Paint trav-map rectangles centered on each door's world position (clipped to wall-free mask)."""
    trav = env.scene.trav_map
    fmap = trav.floor_map[floor]
    map_sz = fmap.shape[0]
    ...
    for door in doors + sliding:
        ...
        og.log.info(f"Painted doorway for {door.name} at map[{r0}:{r1},{c0}:{c1}] (clipped)")
        count += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="180s autonomous nav demo — Benevolence_1_int (same stack as autonomous_nav_60s.py)."
    )
    parser.add_argument(
        "--short",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=_REPO_ROOT / "output" / "autonomous_nav_benevolence1_fpv.mp4",
        help="Output path for the recorded FPV video (default: output/autonomous_nav_benevolence1_fpv.mp4).",
    )
